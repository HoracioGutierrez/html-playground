...
const Draggable = ({
  ...
  return (
    <Badge
      className={cn(
        isLast && "mr-auto",
        "text-accent-foreground  bg-amber-300 text-sm shadow-close hover:text-white hover:bg-emerald-950 transition-[border-radius,background-color] duration-200 rounded-xl hover:rounded-sm group badge hover:animate-pulse"
      )}
      style={style}
      {...listeners}
      {...attributes}
